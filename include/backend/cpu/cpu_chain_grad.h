...
#define VAR_OUT_GRAD(a,m2,k) var_out_grad_buf[a * var_out_grad.out_shape.size() + m2 *K + k]
#define VAR_I_GRAD(a,m2,n) var_i_grad_buf[a * var_i_grad.out_shape.size() + m2 *N + n]
...
#define VAR_OUT_GRAD(a,m,k2) var_out_grad_buf[a * var_out_grad.out_shape.size() + m *K + k2]
#define VAR_I_GRAD(a,n,k2) var_i_grad_buf[a * var_i_grad.out_shape.size() + n *K + k2]
				for (uint64_t a = 0; a < var_i_grad.in_shape.size(); a++) {
...
#define VAR_I_GRAD(a,m,n2) var_i_grad_buf[a *var_i_grad.out_shape.size() + m *N + n2]
					for (uint64_t a = 0; a < var_i_grad.in_shape.size(); a++) {
						for (uint64_t m = 0; m < M; m++) {
